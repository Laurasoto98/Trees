...
public class IntTree {
	...
	public String getLevel(int level) {
		...
		int currentLevel = 1;
		return getLevel(overallRoot, currentLevel, level);
	}
	...
	public void makePerfect() {
		...
		int currentLevel = 1;
		int totLevel = totalLevel(overallRoot);
		makePerfect(overallRoot, currentLevel, totLevel);
	}

	private void makePerfect(IntTreeNode root, int currentLevel, int totLevel) {
		if (currentLevel == totLevel) {
			return;
		}
		if (root.left == null) {
		...
		makePerfect(root.left, currentLevel + 1, totLevel);
		makePerfect(root.right, currentLevel + 1, totLevel);
	}

	private int totalLevel(IntTreeNode root) {
		if (root == null) {
			return 0;
		}
		return 1 + Math.max(totalLevel(root.left), totalLevel(root.right));
	}
